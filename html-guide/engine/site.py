...
article (engine/cssscope.py), the language font tokens, the fonts, the
exercises' script, MathJax, and the PDF manual.
...
from .manifest import RUNTIME_DIR
from .studio import (GUIDE, ROOT as STUDIO_ROOT, MANUAL,
                     find_font, htmlgen, inline_seam, languages, source, texgen)
...
class Site:
    ...
    def _write(self, out, root, order):
        ...
        with inline_seam(self._prepare_inline):
            for k, page in enumerate(order):
                ...
                html_text, entry = self._page(page, root, prev_page, next_page, manual)
                dest = out / page.out
        ...
        (out / "nav.js").write_text(
            ...
            % json.dumps({"title": self.title, "tree": nav_tree,
                          "manual": ("site/" + manual) if manual else None},
    ...
    # ------------------------------------------------------------ one page
    def _page(self, page, root, prev_page, next_page, manual):
        ...
        doc = self._template(page, root, body, toc, prev_page, next_page, ctx, L, manual)
        return doc, entry
    ...
    def _template(self, page, root, body, toc, prev_page, next_page, ctx, L, manual):
        here = page.out
        ...
        manual_href = rel_url(here, manual) if manual else ""
        return PAGE % {
            ...
            if crumbs else "",
            "manual": ('<a class="g-pdf" data-guide-manual href="%s" target="_blank" rel="noopener" '
                       'title="The PDF manual">PDF<span class="g-pdf-more"> manual</span></a>'
                       % esc(manual_href)) if manual else "",
    ...
    def _runtime(self, out):
        """site/_parseh/: what the pages draw with -> the manual's address
        (site/-relative), or None."""
        run = out / RUNTIME_DIR
        (run / "mathjax").mkdir(parents=True, exist_ok=True)
        css = scope_css(source("markdown/app/static/app.css").read_text(encoding="utf-8"))
        (run / "studio.css").write_text(
            "/* generated by html-guide/build.py from markdown/app/static/app.css, every "
            "rule confined to the guide's article (engine/cssscope.py) -- do not edit */\n"
            + css, encoding="utf-8")
        ...
        manual = STUDIO_ROOT / MANUAL
        if manual.is_file():
            shutil.copyfile(manual, run / "manual.pdf")
            return RUNTIME_DIR + "/manual.pdf"
        return None
