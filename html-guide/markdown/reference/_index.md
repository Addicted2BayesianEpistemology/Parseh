...
linkTitle: Reference
weight: 90
description: What a message means, every command for those who want one, where each file lives, the daily loops, the words Parseh uses, and what is new.
---

The other sections of this guide walk through Parseh one door at a time.
This one is for looking things up: you arrive here with a question — a
message on a page you do not understand, a word the other pages use
without explaining it, a file you want to find on the disk — and leave
with the answer.
...
or a video put back from the trash — are on the [troubleshooting
page](troubleshooting.md), where each comes up, with what to do by hand.
The two command pages are for those who like a terminal, or who are
mending something by hand.

## Two things worth knowing first

**The page says why.** When Parseh cannot do something — a recording
that cannot be cut, a PDF that is behind its text, a name that is already
taken — it says so on the page, in a note under the greyed-out button, in
a message, in a badge or in a dialog, in a full sentence. That sentence is
