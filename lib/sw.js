...
/* Parseh — the service worker of the mobile interface (docs/mobile.md, §19).
   ...
   right together.
     ...
     - THE DEADLINE.  `fetch` waits for a socket, not for a computer.  With a
       ...
       it -- the activity poll lib/keep.js asks to draw the offline chip is
       one -- waits a great deal longer (PATIENT), because nothing could take
       its place and some doors are honestly slow; but it waits, and does not
       hang.
   ...
   word up, the translation, an UNKEPT note, the activity poll and every door
   that writes (§19.6 -- the owner chose to keep no dictionary and no
...
// network that is up, the socket settles neither way, and lib/keep.js's own
// probe -- `/__activity`, which is exactly such a door -- would then never
// come back, so the offline chip that is the whole point of the probe would
// never be drawn.  It is generous rather than short because the failure at
// this end is the opposite one: some doors are honestly slow (a waveform
// drawn off a film, a deck read after an import), and cutting a slow SUCCESS
// short, with no copy to put in its place, turns a working thing into a
// refusal.  Half a minute is longer than any honest door and far shorter than
// never.
const DEADLINE = 2500;
const PATIENT = 30000;
...
const NEEDS_COMPUTER = [
  /\/__lookup/, /\/__activity$/, /\/mt\//, /\/__prefs$/, /\/notes\//,
...
self.addEventListener('message', e => {
  ...
  if (msg.warm) e.waitUntil(warm(msg.warm));
...
async function whole(res, want) {
  ...
  if (/^[0-9a-f]{64}$/.test(digest) && self.crypto && self.crypto.subtle) {
    ...
    return hex(await self.crypto.subtle.digest('SHA-256', body)) === digest;
  }
...
async function check(job, reply) {
  ...
  for (const w of want) {
    ...
    try {
      ...
      here = !!res;
      if (res) sound = await whole(res, w);
    } catch (err) {
    ...
    urls.push({url: w.url, here: here, whole: here && sound});
  }
...
async function settle(e, how) {
  ...
  for (const r of records) {
    ...
    try {
      await ((await isShared(url)) ? shared : cache).put(new Request(url), res);
      out.put.push(url);
    } catch (err) {
      out.failed.push(url);
...
function isDoor(url) {
  return /\/api\//.test(url.pathname) || /\/__(offline|shell|activity)$/.test(url.pathname);
}
  ...
  fetch(new Request(url, {cache: 'no-store', credentials: 'same-origin'})).then(async res => {
    ...
    for (const k of keys) {
      ...
      if (k === SHELL && nav) {
        ...
        const now = await res.clone().text();
        await c.put(new Request(url), res.clone());
...
async function answer(r, url) {
  ...
  if (isDoor(url)) {
    ...
    try {
      ...
      // way.  `/__activity` is such a door, and it is what lib/keep.js asks
      // to decide whether the computer can be reached: left to hang, the
      // probe never came back, the offline chip was never drawn, and every
      // page went on behaving as though all were well.  PATIENT is generous
      // because the other failure is real too: a door that is honestly slow
      // and would have succeeded must not be cut short when nothing can take
      // its place.
