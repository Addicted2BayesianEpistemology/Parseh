 ...
 *   ParsehWordline.readingFrom(line, lang) -> the reading its words spell
 *   ParsehWordline.seed(chunk, lang)  -> [field, reading] a draft seeds it with
 *
...
(function (root) {
  ...
  // lib/wordline.py's reading_from, the same rules: the chunk's reading as
  // its words spell it -- the kana run together for a language with a
  // reading, the words' pinyin parted by spaces for one without, the
  // punctuation where the text has it (in pinyin as the ASCII marks tr is
  // written with).  What lib/draft.py seeds a chunk's reading with, so a page
  // can tell that proposal from somebody's writing (seed, below).  '' when
  // the line does not parse or a word in characters has no reading.
  var LATIN_MARK = {'。': '.', '．': '.', '，': ',', '、': ',', '？': '?', '！': '!',
    '：': ':', '；': ';', '…': '...', '“': '"', '”': '"', '「': '"', '」': '"',
    '‘': "'", '’': "'", '『': "'", '』': "'", '（': '(', '）': ')', '《': '"', '》': '"'};
  var OPENING = '“‘「『（《〈【〔';
  function latinMarks(s) {
    return Array.from(s.split('……').join('…')).map(function (ch) {
      return Object.prototype.hasOwnProperty.call(LATIN_MARK, ch) ? LATIN_MARK[ch] : ch;
    }).join('');
  }
  function readingFrom(line, lang) {
    var pairs;
    try { pairs = typeof line === 'string' ? parse(line) : line; } catch (_) { return ''; }
    var kana = !!(lang && lang.reading), out = '', glued = true;
    for (var i = 0; i < pairs.length; i++) {
      var s = pairs[i][0], r = pairs[i][1];
      if (!r && HAN.test(s)) return '';
      var said = r || Array.from(s).filter(function (ch) { return !isSpace(ch); }).join('');
      if (kana) { out += said; continue; }
      if (!r && Array.from(said).every(function (ch) { return PUNCT.test(ch) || trails(ch); })) {
        var opening = Array.from(said).every(function (ch) { return OPENING.indexOf(ch) >= 0; });
        out += (opening && !glued ? ' ' : '') + latinMarks(said);
        glued = opening;
        continue;
      }
      out += (glued ? '' : ' ') + (r ? said : latinMarks(said));
      glued = false;
    }
    return out;
  }
  // lib/wordline.py's seed: [field, reading] -- the field a draft fills from a
  // chunk's word line ('kana' with a reading, 'tr' without) and what it fills
  // it with; [null, ''] when the chunk has no line to read.
  function seed(chunk, lang) {
    if (!lang || !lang.words || !chunk || typeof chunk.words !== 'string' || !chunk.words.trim())
      return [null, ''];
    return [lang.reading ? 'kana' : 'tr', readingFrom(chunk.words, lang)];
  }
  ...
  root.ParsehWordline = {
    ...
    trailing: trailing, aloud: aloud, readingFrom: readingFrom, seed: seed
  };
