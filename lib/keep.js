...
/* Parseh — "Keep on this phone", and knowing when the computer is away
   ...
   The worker (lib/sw.js) holds what is kept; this is the side a person sees:
     ...
     - THE CHIP: every page of the app says whether the computer can be
       ...
       "offline" chip in the bar, which opens the list of what is kept, and
       ↻ beside it, which opens the page afresh; while an answer is slow in
       coming, a quiet "checking…" (the owner, 2026-09-24).  Whether the
       computer is there is asked ONCE per page, by lib/activity.js, and
       "offline" is a verdict it reaches on evidence -- never one slow answer
       (TO-DO §2.24).
     - AND A PAGE OPENS IN THE STATE THE LAST ONE WAS IN (his 5, 2026-09-23:
       ...
       answer is written down under `parseh_away`, and the next page reads
       ...
       pretending for a moment that all is well.  It is a MEMORY and it is
       treated as one: older than a few minutes it is not trusted at all, the
       question behind it corrects it either way, and a page that is
       REFRESHED does not act on it at all.
...
(function () {
  ...
  function sheet(rec, at, have, holds) {
    ...
    function checked(list) {
      ...
      (list || []).forEach(function (x) {
        if (!x || !x.url) return;
        sound[x.url] = !!x.whole;
        // UPDATED IS WHOLE (TO-DO §13.16): the copy here is the file as the
        // computer gave it when it was kept, or as it gives it now, and the
        // worker has written down which (lib/sw.js, check).  It is counted to
        // be SAID, and nothing is fetched for it: kept things can be big.
        if (x.here && x.whole && x.updated) updated++;
      ...
      if (checking) {
        ...
        if (rest)
          what.push((tornRows ? 'The rest are' : 'They are') +
                    ...
                    'without asking.');
        // AND AN UPDATE IS SAID AS ONE, beside whichever of the two the
        // line says: Parseh changing its own files is not the phone losing
        // them, and a sheet that called it damage was crying wolf the morning
        // after every release
        var news = !updated ? ''
          : ' ' + (updated === 1
              ? 'One of its files was updated on the computer since it was kept: the copy ' +
                'here is whole, and nothing is fetched again for it.'
              : updated + ' of its files were updated on the computer since they were kept: ' +
                'the copies here are whole, and nothing is fetched again for them.');
  ...
  /* WHAT THE COMPUTER SAID EACH FILE WAS, by address, as the record gives it:
     handed to the worker with every keep, which writes each on the copy it
     puts (lib/sw.js, `stamped`).  It is what lets the keep check tell a file
     Parseh has UPDATED since it was kept from one that is broken (TO-DO
     §13.16): measured against the computer's digest alone, every kept copy
     of a script a new release changed looked damaged the morning after. */
  function digestsOf(rec) {
    var out = {}, notes = rec.notes || {};
    [rec.small, rec.shared, rec.media, notes.small, notes.shared, notes.media].forEach(function (g) {
      (g || []).forEach(function (x) { if (x && x.url && x.digest) out[x.url] = x.digest; });
    });
    return out;
  }
  ...
  function start(rec, at, urls, bytes, ticked, notes, again) {
    ...
    var job = bgFor(rec)
      ...
      : tell({keep: {id: at.id, urls: urls, version: rec.version || '', renew: !!again,
                     digests: digestsOf(rec)}}, each);
    job.then(function (d) {
      busy = null;
  ...
  function change(rec, at, s) {
    ...
    job.then(function () {
      ...
      return tell({keep: {id: at.id, urls: s.fetch, version: rec.version || '', renew: true,
                          digests: digestsOf(rec)}}, each);
    }).then(function (d) {
      busy = null;
      if (d && d.quiet) { paint(); return; }
  ...
  /* AND IT STARTS WHERE THE PAGE STARTED.  The mark is on <html> before any
     ...
     instead of contradicting it until the first answer comes. */
  var chip = null, again = null, wait = null;
  ...
  /* AND A PAGE THAT OPENED OFFLINE STAYS OFFLINE (the owner's rule of
     ...
     lib/mobile.py) and probes again, which is the way back to online -- and
     so does refreshing it, below. */
  /* ASSUMED, THEN CONFIRMED -- and only the confirming binds.
     ...
     first half of the rule.  The one question (lib/activity.js) then
     answers, and what happens is the difference between the rule's two
     halves:

       it says away           -> confirmed, and this page is an offline page
       ...
       it says there          -> the memory was stale and the computer is
                                 back.  The page corrects itself, in its
                                 first breath and not mid-use -- which is
                                 exactly what "if I want the online version I
                                 get out and get back in" has to mean.
                                 Clamped instead, going back in would have
                                 given an offline page for another three
                                 minutes and the way back would have been
                                 shut.

     AND "AWAY" IS NOW A VERDICT, NOT A GUESS (TO-DO §2.24, the owner,
     2026-09-24: "We should not talk of offline for simply having a slow
     connection").  It used to be one probe that did not answer within three
     seconds -- and since a confirmed away is for the page's whole life, one
     slow answer became an offline page that stayed offline.  It is confirmed
     now only by what lib/activity.js calls evidence: refusals at once, three
     in a row; no network and an ask that fails; or three asks across 45 s
     with not one answered.  The clamp is exactly as it was; what may set it
     is not.

     The mark says which of the two it is -- `assumed` from the page before,
     `confirmed` by this one -- because a write may ask it how sure: an away
     only assumed does not refuse a write, a confirmed one does (static/app.js
     `api`, static/decks.js `call`, lib/parseh.js `ask`). */
  var confirmed = false;
  /* "CHECKING…" (the owner's choice, 2026-09-24).  Slow is online, and the
     page behaves as online throughout -- its cards, its writes -- but a
     person waiting for a computer that has not answered is told so, quietly,
     where the offline chip would stand: only once the quiet has lasted five
     seconds, so a page that is answered promptly never shows it.  A page
     REFRESHED after being offline shows it from its first moment (`start_`),
     so it never looks online by mistake while its question is out. */
  var waiting = false;
  function paintAway() {
    ...
    if (away) document.documentElement.setAttribute('data-parseh-away', confirmed ? 'confirmed' : 'assumed');
    else document.documentElement.removeAttribute('data-parseh-away');
    refresh();
    paint();
    chips();
  }
  /* THE CHIP, AND ↻ BESIDE IT (the owner's choice, 2026-09-24).  "offline"
     opens what is kept here, as it always did; ↻ is a button of its own that
     opens this page afresh -- a refresh, which never inherits what the page
     before found (AWAY_BOOT), so it is the way back the moment the computer
     is back.  On an iPhone or an iPad it is the only way: an app on their
     home screen has no reload button and, as far as is known, no pull-down
     either.  Offline: the chip and ↻.  Waiting: "checking…".  Otherwise
     nothing. */
  function chips() {
    if (away) {
      ...
      if (!again) {
        again = el('button', 'kp-again', '↻');
        again.type = 'button';
        again.title = 'look again: this page is opened afresh, and asks whether the computer is there';
        again.setAttribute('aria-label', 'look again');
    ...
    if (!waiting) { if (wait) { wait.remove(); wait = null; } return; }
    if (!wait) {
      wait = el('span', 'kp-wait', 'checking…');
      wait.title = 'Parseh’s computer has not answered yet: this page goes on as though it were there';
    }
    hang([wait]);
  }
  // still in the page AND still where it can be seen: what was hung before
  // the sheets landed, or before the mode was applied, may be sitting in a
  // bar that is now the hidden one
  function hang(nodes) {
    if (nodes.every(function (n) { return n.isConnected && n.getClientRects().length; })) return;
    var b = bar();
    if (!b || nodes.every(function (n) { return n.parentNode === b; })) return;
    nodes.forEach(function (n) { b.appendChild(n); });
  }
  /* WHAT THE ONE QUESTION FOUND (lib/activity.js, `parseh:reach`).  This file
     asks the computer nothing about whether it is there: that is asked once
     for the whole page, beside the list of what the server is working on,
     and this is where its answer is written down and drawn. */
  var heardLast = '';
  function reached(r) {
    if (!r || !r.state || r.state === 'unknown') return;
    if (r.state === 'slow') {
      if (!waiting) { waiting = true; chips(); }
      return;
    }
    // the same answer again -- the list's own poll comes every few seconds --
    // is written down again and redraws nothing
    var same = r.state === heardLast && !waiting;
    heardLast = r.state;
    waiting = false;
    if (r.state === 'away') {
      found = true;
      ...
      found = false;
      why = '';
      if (!confirmed) away = false;
    }
    // an answer rather than a guess -- which is what the next page will open
    // on (`recall`)
    probed = true;
    noted();
    if (same) return;
    ...
    warmSend();
    if (!away) releaseCheck();
    paintAway();
  }
  /* AND THE NEXT RELEASE IS ASKED FOR, the moment the computer is known to
     be there (TO-DO §13.16: "the next time the app reaches the computer it
     takes the new /sw.js").  The browser looks for a new worker by itself
     only when it sees fit, and it did not see fit: driven in Chromium, a page
     open on a kept book, its worker answering every ask, took no new /sw.js
     through two minutes of navigations to a computer serving one.  So the
     page asks, when the one question has found the computer: the browser
     fetches /sw.js, and a release's new bytes install and take over at once
     (lib/sw.js, `announce`, then `released` here).  Never while the computer
     is away: that would be an ask with nobody to answer.

     BUT NOT ON EVERY PAGE.  The page comes first (the owner's rule for the
     warming, above), and on the tunnel the app is built for every request
     beside a page's own is one it waits behind; an update is a thing of days,
     not of page turns.  So it is asked on the FIRST page each time the app
     is opened (a new window of the app is a new sessionStorage), which is
     when an update made overnight is found, and after that at most once in
     ten minutes, for an app left open all day. */
  var releaseAsked = false, RELEASE_ASKED = 'parseh_release_asked', RELEASE_EVERY = 10 * 60;
  function releaseCheck() {
    if (releaseAsked || !mobile() || !navigator.serviceWorker || !navigator.serviceWorker.controller) return;
    releaseAsked = true;
    var now = Date.now() / 1000, opened = false;
    try {
      opened = !sessionStorage.getItem(RELEASE_ASKED);
      sessionStorage.setItem(RELEASE_ASKED, '1');
    } catch (e) { opened = false; }
    if (!opened && now - (+get(RELEASE_ASKED) || 0) < RELEASE_EVERY) return;
    put(RELEASE_ASKED, String(now));
    navigator.serviceWorker.getRegistration().then(function (r) {
      return r && r.update ? r.update() : null;
    }).catch(function () { /* nobody answered, or the browser said no: the next page asks */ });
  }
  /* HOW THIS PAGE WAS OPENED.  A refresh -- F5, the browser's own button, a
     finger pulling the page down, ↻ -- is a person asking for a fresh answer
     (the owner, 2026-09-24: "upon refreshing, do not assume still offline").
     It is the refreshing that counts, not the way it was asked for, and the
     browser reports every one of them as "reload"; the older
  ...
  var found = away, why = '';
  // `why` rides along -- refused, no network, silent -- for whoever wants to
  // know what kind of away the page before met
  function noted() {
    put(AWAY, JSON.stringify({at: Date.now() / 1000, away: found, trusted: TRUSTED, why: why}));
  }
  ...
  /* THE ASK.  `warmAsked` is what has been sent on this page and `warmWant`
     ...
     dropped, and goes with the next answer that finds the computer, or the
     ...
     the first answer had come back.  Nothing is sent until the question has
     answered once. */
  ...
  function warmSaw(p, over) {
    ...
    // fetch, so the asking is opened again here and the next answer that finds
  ...
  /* ---- Parseh was updated (TO-DO §13.16) ---- */
  /* THE APP IS TOLD, IN A LINE, AND NOTHING IS RELOADED (the owner,
     2026-09-24: never a silent swap under a thumb mid-use).  Every release
     is a new worker, because the server writes the release into /sw.js as it
     serves it (lib/mobile.py, worker()); the new worker takes over the open
     pages the moment it activates, and a person reading a book would
     otherwise have no idea the Parseh under it had changed.  So the page says
     so once, and goes on exactly as it was.

     ASKED AS WELL AS TOLD.  The worker tells the pages that are open when it
     activates (`updated`); a page opened later -- the next morning, the app
     having been closed -- was not there to hear it, so every mobile page asks
     which release the worker is (`release`) and compares it with the one this
     phone last knew (`parseh_release`).  Said once per release for the whole
     phone: the first page to learn it writes it down, and the rest find it
     written.  A phone installing the app for the first time has nothing
     written and a worker that replaced nobody, and is told nothing -- there
     was nothing to update.

     AND IT SAYS WHICH WAY IT WENT.  An update may go back to an older
     version -- that is how a bad release is escaped -- and a line calling
     that "updated" would pass a downgrade off as ordinary, which the owner
     ruled out for the updater itself.  Compared by the same rule
     lib/version.py writes down: the stage letter first (a before b before
     none), then the numbers as numbers, a missing number a zero. */
  var RELEASE_SEEN = 'parseh_release';
  function vkey(v) {
    var m = /^([ab]?)(\d+(?:\.\d+)*)$/.exec(v || '');
    if (!m) return null;
    return [m[1] === 'a' ? 0 : m[1] === 'b' ? 1 : 2].concat(m[2].split('.').map(Number));
  }
  function older(a, b) {
  ...
  function released(r, live) {
    if (!r || !r.version || !mobile()) return;
    var now = r.version + (r.build ? '+' + r.build : '');
    var seen = get(RELEASE_SEEN);
    if (seen === now) return;
    put(RELEASE_SEEN, now);
    // a browser that will keep nothing for this page would say it on every
    // page it opens: there, only the worker's own word at activation is said
    if (!live && get(RELEASE_SEEN) !== now) return;
    if (!seen && !r.replaced) return;
    var was = seen ? seen.split('+')[0] : (r.previous || '');
    announce(older(r.version, was)
      ? 'Parseh went back to ' + r.version + ', an older version than this phone had'
      : 'Parseh was updated to ' + r.version);
  }
  function askRelease() {
    var w = worker();
    if (!w || !mobile()) return;
    try { w.postMessage({release: 1}); } catch (e) { /* no worker to ask: nothing to say */ }
  }
  // THE LINE, where a person looks: the toolbox's toast, held longer than a
  // toast usually is, since it is news nobody asked for; and on the studio's
  // pages, which have no toast this script can reach, a line of its own at
  // the same place, gone on a tap or after the same while
  function announce(m) {
    var p = P();
    if (p && p.toast) { p.toast(m, false, 8000); return; }
    var line = el('p', 'kp-updated', m);
    line.setAttribute('role', 'status');
    line.style.cssText = 'position:fixed;left:50%;bottom:26px;transform:translateX(-50%);' +
      'z-index:2147483000;margin:0;padding:10px 16px;border-radius:10px;max-width:calc(100vw - 32px);' +
      'background:#2b2b2b;color:#fff;font:15px/1.4 system-ui,sans-serif;' +
  ...
  function start_() {
    /* THE STATE THE LAST PAGE WAS IN, BEFORE ANYTHING IS DRAWN (the owner's
       ...
       EXCEPT ON A REFRESH (the owner's fall-back, 2026-09-24): a page opened
       afresh does not act on the memory at all -- it is kept, not erased, and
       the question below writes it again whatever it finds -- and says
       "checking…" until it has an answer, if the memory said away, so that
       it never looks online by mistake meanwhile. */
    var afresh = refreshed();
    var was = afresh ? null : recall();
    if (was) away = was.away;
    if (afresh) {
      var then = recall();
      if (then && then.away) waiting = true;
    }
    at = thing();
    if (at && mobile()) build();
    ...
    if (navigator.serviceWorker) {
      navigator.serviceWorker.addEventListener('message', function (e) {
        ...
        // a new release's worker has just taken this page over, or answers
        // which release it is (see `released`)
        if (d.updated) released(d.updated, true);
        if (d.release) released(d.release, false);
      });
      ...
      navigator.serviceWorker.addEventListener('controllerchange', askRelease);
      askRelease();
    ...
    /* THE TWO ASKS.  The way in on every app page, and the studio's faces
       ...
       answer that finds the computer (`reached`), so neither is ever made while
    ...
    // the one question says whether it is still true
    if (was || waiting) paintAway();
    document.addEventListener('parseh:reach', function (e) { reached(e.detail); });
    // an answer that came before this script was here to hear it
    var A = window.ParsehActivity;
    if (A && A.reach) reached(A.reach());
  }
  ...
  window.ParsehKeep = {reg: reg, big: big, tell: tell, made: made,
