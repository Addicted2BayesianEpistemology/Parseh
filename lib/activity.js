/* Parseh — what the server is working on, shown on every page.
   ...
   the add pages, the lookup page, the Anki wizard, the clip tray), and the
...
(function () {
  ...
  var timer = null, inflight = false, dead = false, fails = 0, hot = 0;
  ...
  function poll() {
    timer = null;
    if (inflight || dead || offDisk) return;
    inflight = true;
    fetch('/__activity', {cache: 'no-store', credentials: 'same-origin'})
      .then(function (r) {
        // no list on this server (a studio run on its own): stop asking
        if (r.status === 404) { dead = true; return null; }
        return r.json();
      })
      .then(function (j) {
        fails = 0;
      ...
      .then(function () {
        inflight = false;
        schedule(fails ? Math.min(30000, SLOW * Math.pow(2, fails - 1)) : busy() ? FAST : SLOW);
      });
  }
