...
(function () {
  ...
  try {
    ...
    if (seen && seen.away === true && typeof seen.at === 'number' &&
        Date.now() / 1000 - seen.at <= (typeof seen.trusted === 'number' ? seen.trusted : 180))
      document.documentElement.setAttribute('data-parseh-away', '');
  } catch (e) {}
  ...
  /* AND AN ASK THAT CANNOT HANG, for the pages this file is the only shared
     ...
     fix the phone.  So the ask is left alone and the one cheap question is
     asked BESIDE it every few seconds -- is anybody there?  A computer that
     answers goes on being waited for; a computer that does not is gone, and
     the ask fails at once with `away` set, which is what the callers show a
     sentence about instead of a spinner. */
  var WATCH = 3000;
  function awayNow() { return document.documentElement.hasAttribute('data-parseh-away'); }
  function nobodyThere() {
    return Promise.race([
      fetch('/__activity', {cache: 'no-store'}).then(function (r) {
        return !r.ok && r.status === 503;
      }).catch(function () { return true; }),
      new Promise(function (done) { setTimeout(function () { done(true); }, WATCH); }),
    ]);
  }
  function ask(url, init) {
    init = init || {};
    var writes = (init.method || 'GET').toUpperCase() !== 'GET';
    if (writes && awayNow() && !init.evenAway) {
    ...
    return new Promise(function (yes, no) {
      var settled = false;
      fetch(url, init).then(function (r) { if (!settled) { settled = true; yes(r); } },
                            function (e) { if (!settled) { settled = true; e.away = true; no(e); } });
      function look() {
        if (settled) return;
        nobodyThere().then(function (gone) {
          if (settled) return;
          if (!gone) { setTimeout(look, WATCH); return; }
          settled = true;
