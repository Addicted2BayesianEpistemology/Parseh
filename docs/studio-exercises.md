...
A document's PDF prints the exercises without their answers marked: a blank is left empty, no choice is ticked and no statement is marked true or false, and the explanations are left out, as are the pictures and recordings of the answer. A flashcard is the exception: it prints both its sides, `Front:` and `Back:`, as a card to study from. The exercises a student writes on are laid out for a pen:
