...
It is being built one interface at a time, and today \textbf{the hub is the
...
each with its counts, and \menu{Guide}. Nothing on it administers anything:
...
comes back until it is answered right. At the end, \menu{Shuffle and repeat}
deals them again and \menu{Back to the deck} returns to the list, the
selection still there. \textbf{Cramming never touches the deck's schedule}
